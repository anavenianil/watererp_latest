...
/**
 * Performance test for the ValveComplaint entity.
 */
class ValveComplaintGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ValveComplaint entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all valveComplaints")
            .get("/api/valveComplaints")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created valveComplaint")
                .get("${new_valveComplaint_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created valveComplaint")
            .delete("${new_valveComplaint_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
