...
public class BillRunMasterResource {
    ...
    @Timed
    public ResponseEntity<BillRunMaster> createBillRunMaster(@RequestBody BillRunMaster billRunMaster) throws URISyntaxException {
        log.debug("REST request to save BillRunMaster : {}", billRunMaster);
        if (billRunMaster.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("billRunMaster", "idexists", "A new billRunMaster cannot already have an ID")).body(null);
        }
        BillRunMaster result = billRunMasterRepository.save(billRunMaster);
        return ResponseEntity.created(new URI("/api/billRunMasters/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("billRunMaster", result.getId().toString()))
            .body(result);
    }
