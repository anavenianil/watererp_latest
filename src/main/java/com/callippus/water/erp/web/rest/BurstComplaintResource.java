...
public class BurstComplaintResource {
     ...
     */
    @RequestMapping(value = "/burstComplaints/getByComplaintId",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
	public ResponseEntity<BurstComplaint> getBurstComplaintByComplaintId(@Param("waterLeakageComplaint") Long waterLeakageComplaint)
			throws Exception {
    	...
    	BurstComplaint burstComplaint = burstComplaintRepository.findByWaterLeakageComplaint(waterLeakageComplaintRepository.findOne(waterLeakageComplaint));
    
    	return Optional.ofNullable(burstComplaint)
				.map(result -> new ResponseEntity<>(burstComplaint, HttpStatus.OK))
				.orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
	}
}
