...
public class HydrantComplaintResource {
    ...
    /**
     * Get BurstComplaint by complaintId
     */
    @RequestMapping(value = "/hydrantComplaints/getByComplaintId",
            method = RequestMethod.GET,
            produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
	public ResponseEntity<HydrantComplaint> getHydrantComplaintByComplaintId(@Param("waterLeakageComplaint") Long waterLeakageComplaint)
			throws Exception {
    	log.debug("REST request to BurstComplaint by complaintId : {}");
    	
    	//WaterLeakageComplaint waterLeakageComplaint = waterLeakageComplaintRepository.findOne(complaintId);
    	HydrantComplaint hydrantComplaint = hydrantComplaintRepository.findByWaterLeakageComplaint(waterLeakageComplaintRepository.findOne(waterLeakageComplaint));
    
    	return Optional.ofNullable(hydrantComplaint)
				.map(result -> new ResponseEntity<>(hydrantComplaint, HttpStatus.OK))
				.orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
	}
}
